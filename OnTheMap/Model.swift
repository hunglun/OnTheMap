            ...
            do {
                if let parsedResult = try NSJSONSerialization.JSONObjectWithData(data!, options: .AllowFragments) as? NSDictionary,
                    results = parsedResult["results"] as? [[String : AnyObject]]{
                        ...
                        StudentModel.sharedInstance().studentLocations = []
                        for entry in results {
                            if let student = StudentModel.StudentLocation(studentInformation: entry) {
                                StudentModel.sharedInstance().studentLocations?.append(student)
                            }else {
